...
static void ide_read(struct disk* hd, unsigned int lba, void* buf, unsigned int sec_cnt)
{ 
    ASSERT(lba <= max_lba);
    ASSERT(sec_cnt > 0);
...
static void ide_write(struct disk* hd, unsigned int lba, void* buf, unsigned int sec_cnt)
{
    ASSERT(lba <= max_lba);
    ASSERT(sec_cnt > 0);
    lock_acquire(&hd->my_channel->lock);
